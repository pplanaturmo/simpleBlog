<nav x-data="{ open: false }" class="bg-white border-b border-black">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ">
        <div class="flex justify-between h-16">
            <div class="flex w-2/3 space-around">
                <!-- Logo -->
                ...
                <!-- Navigation Links -->
                <div class="hidden space-x-4 sm:flex flex-grow justify-center">

                      <x-nav-link :href="route('blog')" :active="request()->routeIs('blog')">
                        {{ __('Home') }}
                    </x-nav-link>
                    @can('create posts')<!-- Permisos-->
                    <x-nav-link :href="route('posts.create')" :active="request()->routeIs('posts.create')">
                        {{ __('Create Post') }}
                    </x-nav-link>
                    @endcan
                    <x-nav-link :href="route('categories.index', ['categoryId' => 'all'])" :active="request()->routeIs('categories.index')">
                        {{ __('Categories') }}
                    </x-nav-link>
                    @can('manage users')<!-- Permisos-->
                    <x-nav-link :href="route('users.index')" :active="request()->routeIs('users.index')">
                        {{ __('Users Management') }}
                    </x-nav-link>
                    <x-nav-link :href="route('jsons')" :active="request()->routeIs('jsons')">
                        {{ __('JSON Page') }}
                    </x-nav-link>
                    @endcan
                </div>
